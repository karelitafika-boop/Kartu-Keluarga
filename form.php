...
<div class="form-wrapper">
    <div class="form-card">
        ...
        <form id="formKK" method="POST" enctype="multipart/form-data">
            ...
            <input type="text" name="no_kk" class="form-control" placeholder="No KK" value="<?= $_POST['no_kk'] ?? '' ?>" required>
            <input type="text" name="nama_kepala_keluarga" class="form-control" placeholder="Nama Kepala Keluarga" value="<?= $_POST['nama_kepala_keluarga'] ?? '' ?>" required>
            ...
            <?php } else { ?>
            <input type="text" class="form-control" value="File KK sudah dipilih" readonly>
            <?php } ?>
            ...
            <?php } else { ?>
            <input type="text" class="form-control" value="File akta lahir sudah dipilih" readonly>
            <?php } ?>
            ...
            <?php } else { ?>
            <input type="text" class="form-control" value="File akta perkawinan sudah dipilih" readonly>
            <?php } ?>
            
            <textarea name="alamat" class="form-control" placeholder="Alamat" required><?= $_POST['alamat'] ?? '' ?></textarea>
            <?php if($step == 1) { ?>
            
            <h3 class="section-heading">Jumlah Anggota</h3>
            <select name="jumlah_anggota" class="form-select" required>
                <option value="">Pilih jumlah anggota</option>
                <?php for($i=1; $i<=10; $i++) { ?>
                <option value="<?= $i ?>"><?= $i ?> Anggota </option>
                <?php } ?>
            </select>
            
            <button type="submit" name="step" value="2" class="btn-save">Lanjut</button>
            ...
            <input type="hidden" name="jumlah_anggota" value="<?= $jumlahAnggota ?>">
            <h3 class="section-heading">Anggota Keluarga</h3>
            <?php for($i=1; $i<=$jumlahAnggota; $i++) { ?>
            <div class="anggota-box">
                <h4>Anggota <?= $i ?></h4>
                <input type="text" name="nik[]" class="form-control" placeholder="NIK" value="<?= $_POST['nik'][$i-1] ?? '' ?>" required>
                <input type="text" name="nama[]" class="form-control" placeholder="Nama" value="<?= $_POST['nama'][$i-1] ?? '' ?>" required>
                
                <select name="jenis_kelamin[]" class="form-select" required>
                    <option value="">Pilih Jenis Kelamin</option>
                    <option value="Laki-laki" <?= (($_POST['jenis_kelamin'][$i-1] ?? '') == 'Laki-laki') ? 'selected' : '' ?>>
                        Laki-laki
                    </option>
                    <option value="Perempuan" <?= (($_POST['jenis_kelamin'][$i-1] ?? '') == 'Perempuan') ? 'selected' : '' ?>>
                        Perempuan
                    </option>
                </select>
                
                <input type="text" name="hubungan[]" class="form-control" placeholder="Hubungan" 
                value="<?= $_POST['hubungan'][$i-1] ?? '' ?>" required>
            </div>
            
            <?php } ?>
            
            <div class="button-area">
                <a href="form.php" class="btn-add" style="text-decoration:none; display:flex; align-items:center;">
                    ← Kembali
                </a>
            ...
            <div style="
                ...
                <h3 style="font-weight:800; color:#0f172a; margin-bottom:10px;">Periksa kembali data kamu</h3>
                <p style="color:#64748b; font-size:14px;">Pastikan semua data sudah benar sebelum disimpan.</p>
                <div style="display:flex; gap:10px; justify-content:center; margin-top:20px; flex-wrap:wrap;">
                    <button type="submit" name="step" value="2" class="btn-add">Ulangi</button>
                    <button type="submit" name="step" value="4" class="btn-save">Simpan Data</button>
                </div>
            </div>
